...
# ============================================
# Linux ADB helpers (native adb only)
# ============================================
...
def get_adb_info() -> dict:
    ...
    try:
        ...
        return {"available": result.returncode == 0, "path": adb,
                "platform": "Linux", "version": output.splitlines()[0] if output else "",
                "error": None if result.returncode == 0 else output.strip()}
    except (OSError, subprocess.SubprocessError) as exc:
        return {"available": False, "path": adb, "platform": "Linux", "error": str(exc)}

# compatibility names used by the app
find_adb = _find_adb
get_adb_info_compat = get_adb_info

# ============================================
# Constants / runtime YTS
# ============================================
CACHE_TIMEOUT = 60
MAX_RETRIES = 2
TEST_TIMEOUT = 300
...
def init_db():
    with get_db() as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS test_results (
            ...
            device_id TEXT, short_id TEXT, status TEXT NOT NULL,
            timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, duration INTEGER, logs TEXT)''')
        conn.execute('''CREATE TABLE IF NOT EXISTS test_metrics (
            id INTEGER PRIMARY KEY AUTOINCREMENT, test_name TEXT UNIQUE,
            total_runs INTEGER DEFAULT 0, passes INTEGER DEFAULT 0,
            failures INTEGER DEFAULT 0, avg_duration REAL DEFAULT 0)''')
        conn.commit()
    ...
    try:
        with get_db() as conn:
            ...
            if existing:
                ...
                passes = existing['passes'] + (status == 'PASSED')
                failures = existing['failures'] + (status == 'FAILED')
                avg = ((existing['avg_duration'] * existing['total_runs']) + (duration or 0)) / total
                conn.execute('UPDATE test_metrics SET total_runs=?, passes=?, failures=?, avg_duration=? WHERE test_name=?',
                             (total, passes, failures, avg, test_name))
            else:
...
def get_results_db(limit=100):
    with get_db() as conn:
        rows = conn.execute('SELECT * FROM test_results ORDER BY timestamp DESC LIMIT ?', (limit,)).fetchall()
        return [dict(row) for row in rows]

def get_stats_db():
    with get_db() as conn:
        rows = conn.execute('''SELECT test_name,total_runs,passes,failures,ROUND(avg_duration,2) as avg_duration,
            ROUND(passes*100.0/total_runs,1) as pass_rate FROM test_metrics ORDER BY total_runs DESC''').fetchall()
        return [dict(row) for row in rows]
...
YTS_MANUAL_TESTS = {"LiveDRM","In-app HDR HLG","In-app HDR PQ","In-app Visual Audio / Video Sync","DRM Purchased Movie","Multi-App Performance","System Overlay","YouTube Music Endurance","Live streaming"}
YTS_TEST_WAIT_SECONDS = {"LiveDRM":1080,"In-app HDR HLG":300,"In-app HDR PQ":300,"YouTube Music Endurance":310,"Live streaming":43200}
TEST_CATEGORIES = {"Media Tests":["Adaptive Bit Rate","Adaptive Bit Rate - DRM","Resizing"],"Aspect Ratio Tests":["21:9 Aspect Ratio","4:3 Aspect Ratio","16:9 Aspect Ratio","17:30 Aspect Ratio"],"ETC Tests":["Fonts","Captions","Localization","TTS","Screensaver","High Contrast Text Setting","TTS Setting","Animated WebP","Non-animated WebP","Page Visibility Overlay","Language Setting"],"Manual Tests":["LiveDRM","YouTube Music Endurance","Live streaming"],"Performance Tests":["Multi-App Performance","System Overlay"],"HDR Tests":["In-app HDR HLG","In-app HDR PQ"],"Launch Tests":["In-app Visual Audio / Video Sync","DRM Purchased Movie","Sign-in Persistence"],"Guided Tests":["Params Remote","System-wide Exit Key","Alternate Time Zone","Params Menu"],"Endurance Tests":["12 Hour Endurance","Persistence Cookie 200 times"],"Voice Tests":["[Voice&Search] Soft-mic Button"],"Time Tests":["Current Time"]}

# Preserve all existing instruction text from the original source through a compact fallback.
...
def prepare_yts() -> str:
    ...
    with _yts_prepare_lock:
        ...
        try:
            urllib.request.urlretrieve(YTS_DOWNLOAD_URL, zip_path)
            _safe_extract_zip(zip_path, extract_dir)
            launcher = _locate_yts(extract_dir)
            if not launcher:
                raise RuntimeError("Downloaded YTS package does not contain a usable launcher")
            if launcher.suffix == "" and not os.access(launcher, os.X_OK):
...
def _track_process(proc):
    with _yts_processes_lock:
        _yts_processes.add(proc)

def _untrack_process(proc):
    with _yts_processes_lock:
        _yts_processes.discard(proc)
    ...
    try:
        while True:
            ...
            if line:
                ...
                if log_callback: log_callback(f"ERROR: Command timed out after {timeout} seconds")
                break
...
for _sig in (getattr(signal, "SIGINT", None), getattr(signal, "SIGTERM", None)):
    if _sig:
        try: signal.signal(_sig, lambda *_: cleanup_yts())
        except Exception: pass

# ============================================
# ADB / Node / YTS helpers
# ============================================
def check_adb_devices():
    target = os.environ.get("ADB_DEVICE", "").strip()
    if target:
        ok, msg = ensure_network_device(target)
        if not ok: logger.warning("ADB connect failed for %s: %s", target, msg)
    try:
        result = subprocess.run(adb_command("devices"), stdout=subprocess.PIPE,
                                stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace", timeout=15, check=False)
        devices=[]
        for line in result.stdout.splitlines()[1:]:
            parts=line.split()
            if len(parts)>=2 and parts[1] in {"device","offline","unauthorized"}: devices.append(parts[0])
        return devices
    except Exception:
        logger.exception("Failed to query ADB devices")
        return []
...
def discover_yts_devices():
    ...
    mapping={}
    for line in lines:
        m_short=re.search(r"\(([^)]+)\)", line)
        m_adb=re.search(r"adb:\s*([^\s)]+)", line)
        if m_short and m_adb: mapping[m_adb.group(1)]=m_short.group(1)
    return mapping

def check_node():
    try:
        result=subprocess.run(["node","--version"],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=10,check=False)
...
def get_device_details_cached(device_id):
    ...
    details={"model":"Unknown","manufacturer":"Unknown","android_version":"Unknown","security_patch":"Unknown","product":"Unknown","fingerprint":"Unknown"}
    props={}
    for key in ("ro.product.model","ro.product.manufacturer","ro.build.version.release","ro.build.version.security_patch","ro.product.name","ro.build.fingerprint"):
        try:
            result=subprocess.run(adb_command("-s",device_id,"shell","getprop",key),stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,text=True,timeout=10,check=False)
            props[key]=result.stdout.strip()
        except Exception: pass
    details.update({"model":props.get("ro.product.model") or "Unknown","manufacturer":props.get("ro.product.manufacturer") or "Unknown","android_version":props.get("ro.build.version.release") or "Unknown","security_patch":props.get("ro.build.version.security_patch") or "Unknown","product":props.get("ro.product.name") or "Unknown","fingerprint":props.get("ro.build.fingerprint") or "Unknown"})
    _device_cache[device_id]=details; _cache_timestamps[device_id]=now
    return details

def _build_yts_args(short_id, test_name):
    template=YTS_TEST_COMMANDS.get(test_name)
    if not template: raise ValueError(f"Unknown test: {test_name}")
    import shlex
    formatted=template.format(shortId=short_id)
    parts=shlex.split(formatted)
    if parts and parts[0] == "yts": parts=parts[1:]
    return parts

def _detect_result(lines):
    for line in reversed(lines):
        up=line.upper()
        if "RESULT: PASSED" in up or "✓ PASSED" in up: return "PASSED"
        if "RESULT: FAILED" in up or "✗ FAILED" in up: return "FAILED"
    return None

def run_test(short_id, test_name, device_id=None, log_callback=None):
    start=time.time(); logs=[]
    try:
        command=_yts_command(*_build_yts_args(short_id,test_name))
        if log_callback: log_callback("Executing: " + " ".join(command))
        rc,lines=_run_command(command, timeout=max(TEST_TIMEOUT, YTS_TEST_WAIT_SECONDS.get(test_name,0)+300), log_callback=log_callback)
        logs=lines
        result=_detect_result(lines) or ("PASSED" if rc == 0 else "FAILED")
    except Exception as exc:
        if log_callback: log_callback(f"ERROR: {exc}")
        result="FAILED"
...
def get_test_stats(): return get_stats_db()
def get_recent_results(limit=100): return get_results_db(limit)
def get_test_categories(): return TEST_CATEGORIES
